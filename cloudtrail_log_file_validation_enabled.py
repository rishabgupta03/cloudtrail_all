#!/usr/bin/env python3
"""
Control: CloudTrail has log file validation enabled.
get_trail returns LogFileValidationEnabled directly. Multi-region trails
are only evaluated once, in their HomeRegion, to avoid double-counting.
"""
...
CONTROL_NAME = "CloudTrail Has Log File Validation Enabled"
...
def evaluate_trail(trail_info):
    ...
    if trail_info.get("LogFileValidationEnabled"):
        return "COMPLIANT", "Log file validation is enabled (digest files are generated for tamper detection)"
    return "NON_COMPLIANT", "Log file validation is disabled - log file integrity cannot be verified"
...
def write_csv(results, account_id):
    ...
    filename = f"cloudtrail_log_file_validation_{account_id}_{timestamp}.csv"
