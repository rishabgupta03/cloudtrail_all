#!/usr/bin/env python3
"""
Control: CloudTrail records all S3 object-level API operations (read and
write) for all S3 buckets.

Two dimensions must both be satisfied for COMPLIANT:
  1. Bucket scope - the S3 data event selector must cover ALL buckets.
  ...
  2. Full read+write coverage - the selector must capture both read and
     write object-level operations, not just one side.
       Classic: ReadWriteType is "All", or separate selectors together
                cover "ReadOnly" and "WriteOnly"
       Advanced: no readOnly field selector (unrestricted, covers both), or
                 both "true" and "false" present across selectors

If S3 data events are logged but only cover reads, only writes, or are
scoped to specific buckets, this is NON_COMPLIANT with evidence naming the
missing dimension. Multi-region trails are only evaluated once, in their
...
CONTROL_NAME = "CloudTrail Records All S3 Object-Level Operations For All Buckets"
...
def evaluate_advanced_selectors(advanced_selectors):
    ...
    if covers_read and covers_write and covers_all_buckets:
        return "COMPLIANT", "Advanced event selector logs all S3 object-level operations (read and write) for all buckets"

    missing = []
    if not (covers_read and covers_write):
        missing.append("full read+write event coverage")
...
def evaluate_basic_selectors(basic_selectors):
    ...
    rw_types = {s.get("ReadWriteType", "All") for s in s3_data_selectors}
    full_rw_coverage = "All" in rw_types or {"ReadOnly", "WriteOnly"}.issubset(rw_types)

    covers_all_buckets = any(
        ALL_BUCKETS_ARN in dr.get("Values", [])
        for s in s3_data_selectors
        for dr in s.get("DataResources", [])
        if dr.get("Type") == "AWS::S3::Object"
    )

    if full_rw_coverage and covers_all_buckets:
        return "COMPLIANT", "Event selector logs all S3 object-level operations (read and write) for all buckets"

    missing = []
    if not full_rw_coverage:
        missing.append(f"full read+write event coverage (found: {', '.join(rw_types)})")
    if not covers_all_buckets:
...
def evaluate_trail_s3_data_events(client, trail_arn):
    """Return (status, evidence) for one trail's full S3 object-level coverage."""
...
def write_csv(results, account_id):
    ...
    filename = f"cloudtrail_s3_object_all_events_{account_id}_{timestamp}.csv"
